...
When testing in general, use the existing placeholder tone tracks (`public/audio/tone-1.wav` through `tone-6.wav`, generated by `scripts/generate-placeholder-audio.mjs`) rather than sourcing new placeholder audio or real tracks. Only use a real track when testing something that specifically requires it (e.g. verifying real audio/cover-art wiring).
