...
const coverVinylFiles = import.meta.glob<string>("/src/assets/albums/*/cover-vinyl.jpg", {
  eager: true,
  import: "default",
  query: "?url",
});
...
function discoverRealAlbums(): Album[] {
  const byFolder = new Map<
    string,
    { cover?: string; coverVinyl?: string; tracks: { file: string; url: string }[] }
  >();
  ...
  return Array.from(byFolder.entries())
    ...
    .map(([folder, entry]) => {
      ...
      return { id, title, artist, coverArt, coverArtVinyl: entry.coverVinyl ?? coverArt, tracks };
    });
}
